...
@export var walll_jump_velocity: float = 200.0
...
#Determine if player has just fallen off a ledge
...
#Let player jump if they pressed jump button after falling off a ledge
func handle_coyote_time(body: CharacterBody2D) -> void:
	if has_just_stepped_off_ledge(body):
		coyote_timer.start()
	
	#Maintain player hovering in the air during coyote time
